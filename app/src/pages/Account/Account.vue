<template>
    <div class="form">
        ...
        >
            <template #buttonDelete>
                <div class="button-template hovered dx-button-text buttonDelete">
                    Удалить аккаунт
                </div>
            </template>
        </DxButton>
        ...
        >
            <template #popup-content>
                <div>
                    <div class="warning">
                        <p>Настоящим Вы соглашаетесь с тем, что Ваша учётная запись будет удалена навсегда, а ранее заключенное Пользовательское соглашение (гражданско-правовой договор об условиях оказания Услуг на Сайте между Пользователем и Компанией) – расторгнуто Вами в одностороннем порядке. Вы больше не сможете выполнять Задания на Сайте и получать Оплату. Компания вправе хранить Ваши персональные данные в своей системе после удаления Вашей учетной записи, если такие данные необходимы для дальнейшей работы Сайта или хранение таких данных является обязательным в соответствии с законодательством.</p>
                    </div>
                    <div class="dx-fieldset">
                        <DxCheckBox
                                id="check"
                                :value="false"
                                text="Я прочитал и хочу удалить аккаунт"
                                @option-changed="check($event)"
                                v-model="checkDeletions"
                                :validation-status="checkboxStatus"
                        >
                        </DxCheckBox>
                        <p class="delete-is-invalid" v-if="showCheckBoxError">Вы должны принять условия!</p>
                    </div>
                    <div class="popup-buttons">
                        <DxButton
                                id="agree"
                                template="buttonAgree"
                                @click="deleteAccount"
                        >
                            <template #buttonAgree>
                                <div class="button-template hovered dx-button-text buttonDelete">
                ...
                >
                    ...
                    >
                        ...
                        >
                            ...
                            />
                            <DxLabel text="Номер банковской карты(для перевода средств)" location="top"/>
                        </DxSimpleItem>
                        <DxSimpleItem
    ...
    export default {
        ...
        data() {
            return {
                ...
                files: [],
                formInstance: null,
                checkDeletions: false,
                checkboxStatus: 'valid',
        ...
        methods: {
            ...
            check(e) {
                this.checkDeletions = e.value;
                if(e.value){
                    this.checkboxStatus = 'valid';
                    this.showCheckBoxError = false;
                } else {
                    this.checkboxStatus = 'invalid';
                    this.showCheckBoxError = true;
                }
            },
            showPopup(){
                this.popupVisible = !this.popupVisible;
                if(this.checkDeletions){
                    this.checkDeletions = false;
                }
            },
            ...
            async deleteAccount(){
                if(this.checkDeletions){
                    const {success, errors} = await personalService.deleteUser();
                    if(success){
                        notify({
        ...
        async mounted() {
            ...
            const {success, data} = await personalService.getPersonal();
            if(success){
                if (typeof data === "object" && Object.keys(data).length !== 0){
                    const fullname = data.fullname.split(' ');
                    const pass = data.passport.split(' ');
                    this.form = data;
                    this.form.dateOfBirth = new Date(data.dob);
                    this.form.passport_date = new Date(data.passport_date);
                    this.form.username = fullname[0];
                    this.form.surname = fullname[2];
                    this.form.patronymic = fullname[1];
                    this.form.passport_series = pass[0];
                    this.form.passport_number = pass[1];
                    this.imgs = data.img;
                }
            }
        }
